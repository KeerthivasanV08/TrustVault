...
import { extractAccounts } from '@/lib/normalizers/accountNormalizer';
...
export async function fetchAccounts(params: { limit?: number; search?: string; risk?: string; offset?: number } = {}): Promise<AccountListResponse> {
  ...
  return {
    total: (raw as any)?.total ?? results.length,
    count: (raw as any)?.count ?? results.length,
    offset: (raw as any)?.offset ?? 0,
    limit: (raw as any)?.limit ?? results.length,
    results,
  };
}

export async function fetchAccountById(id: string): Promise<Account> {
  const raw = await client.request<unknown>({ path: `/api/accounts/${encodeURIComponent(id)}` });
...
export function normalizeAccountById(raw: any): Account {
  ...
  return {
    ...
    final_risk_score: raw?.final_risk_score != null ? Number(raw.final_risk_score) : raw?.finalRiskScore != null ? Number(raw.finalRiskScore) : raw?.onboarding_risk_score != null ? Number(raw.onboarding_risk_score) : undefined,
    confidence: raw?.confidence != null ? Number(raw.confidence) : raw?.confidence_score != null ? Number(raw.confidence_score) : undefined,
